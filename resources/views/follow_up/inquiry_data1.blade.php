								   ...
								   <div class="form-group col-sm-12">
									...
									<div class="form-group col-sm-12">
										...
										<div class="col-sm-4 plant_city_hide">
											...
											<select name="project_city" id="project_city" class="select2 project_city" style="width:100%" >
								...
								@if($role_id == 1)
									...
									<div class="form-group col-sm-12">
										...
										<div class="col-sm-3">
											...
											<center><a id="regret_follow_up" data-toggle="modal" data-target="#send_sms_model" class="btn btn-primary mybtn all_btn_disabled all_btn_hide">Send SMS</a></center>
										   ...
										   <tbody>
											...
											@foreach ($quotation_no as $key=>$value)
												...
												@php
													...
													$print_sample = URL::to('Quatation/simple_quatation',array($utility->encode($value->quatation_id)),false);
												...
												<tr>
													...
													<a target='_blank' class='btn btn-primary btn-flat btn-sm' href='{{ $print_sample }}'><i class='glyphicon glyphicon-print icon-white'></i> Simple Quatation</a>
											 ...
											 @foreach ($revise_quot_num as $key=>$value)
												...
												@php
													...
													$revise_simple_view = URL::to('Quatation/revice_simple_quatation',array($utility->encode($value->revise_id)),false);
												...
												<tr>
													...
													<a target='_blank' class='btn btn-primary btn-flat btn-sm' href='{{ $revise_simple_view }}'><i class='glyphicon glyphicon-print icon-white'></i> Simple Quatation</a>
				 ...
				 <div class="box-body">
					...
					<div class="form-group col-sm-12">
						...
						<div class="col-sm-4">
							...
							<input type="text" readonly value="{{ date('Y') }}" name="acc_year" id="acc_year" class="form-control numberonly" />
				...
				{{ csrf_field() }}
				 ...
				 <div class="form-group col-sm-12">
						...
						<div class="col-sm-9">
							...
							<select name="customer_quot_no" id="customer_quot_no" class="select2" style="width:100%">

								<option  value=''>Select</option>

								@foreach($quotation_no as $k=> $v)

									<option value="{{ $v->quatation_no }}" > {{ $v->quatation_no }}</option>

								@endforeach

								@foreach($revise_quot_num as $key=> $val)

									<option value="{{ $val->revise_quatation_no }}" > {{ $val->revise_quatation_no}}</option>
...
jQuery(document).ready(function($){
	...
	$('body').on('change', '#acc_year, #acc_week', function(e){
		var acc_year = $('body').find('#acc_year').val();
		var acc_week = $('body').find('#acc_week').val();

		if (acc_year != '' && acc_week != '') {
			var date = getDateRangeOfWeek(acc_week,acc_year);
	...
	$('body').on('keyup','#acc_week', function(){
		...
		{
			var date = getDateRangeOfWeek(weak,acc_year);
			var date1 = date.split(",");
			$('#hot_list_from_date').val(date1[0]);
			$('#hot_list_to_date').val(date1[1]);
		}
			...
			$.ajax({
...

				{
...

					{
						...
						alert('Follow Up Detail Not Successfully Saved.Refresh And Try Again !');
	...
	$('#send_sms_form').validate({
		...
		rules: {
			...
			'customer_quot_no':{required:true,},
		...
		messages:{
			...
			'customer_quot_no':{required:"Please select Quotation Number",},
	...
	$('#hot_list_form').on('submit', function(e){
...

		{
...

			{
				...
				alert("Please Select Client Category Hot In Customer Detail");
	...
	// customer detail get state wise city

	$('select#project_state').change(function(){
		var project_state_id = $("#project_state").val();

		$.ajax({
			type: "POST",
			url: "{{ url('get_project_city') }}",
			data: { "_token": "{{ csrf_token() }}",project_state_id:project_state_id},
			success:function (html)
			{
				$("select#project_city").html(html);
			}
		});
