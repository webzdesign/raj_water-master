...
<div class="blockUI blockMsg blockPage" style="z-index: 1011; position: fixed; padding: 15px; margin: 0px; width: 30%; top: 40%; left: 35%; text-align: center; color: rgb(255, 255, 255); border: medium none;  cursor: wait; opacity: 0.5;">
	<img alt="loading.." src="{{ URL::asset('external/loader.gif')}}">
</div>
	...
	$('body').on('click','#quotation_tab_click',function(){
		if (! $.fn.dataTable.isDataTable( '#inquiry_quotation' ) )
		{
			quotation_inq = $("#inquiry_quotation").DataTable({
				...
				autoWidth:false,
				ajax:"{{ $get_inq_quotation_url }}"
				}).on( 'order.dt search.dt', function () {
				$("#inquiry_quotation").DataTable().column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
					cell.innerHTML = i+1;
				} );
			}).draw();
		}
