...
<?php
error_reporting(0);
$url = $controller_name.'@add_revise_quatation';
$btn= "Re - Generate";
       ...
       <div class="box box-primary">
            ...
            <div class="box-header with-border">
              ...
              {!! Form::open(array('action' => $url, 'method' => 'post' , 'files' => true ,'id'=>"frm",'name'=>"frm",'class'=>"form"))!!}
				...
				{!! Form::hidden('quatation_id',$quatation_id, array('id'=>"quatation_id")) !!}



                      <div class="form-group col-sm-12">
                            <div class="form-group col-sm-3">
                            {!! Form::label('Revise Quotation No') !!}
                                 {!! Form::text('revise_quatation_no',$revise_quatation_no, array('class' => 'form-control ' ,'id'=>"revise_quatation_no",'required' => 'required','readonly'=>'readonly')) !!}
                            </div>
                       		<div class="form-group col-sm-3">
                            {!! Form::label('Revise Quotation Date') !!}
                            {!! Form::text('revise_date',date('d-m-Y'), array('class' => 'form-control ' ,'id'=>"revise_date",'required' => 'required','readonly'=>'readonly')) !!}
                            </div>

							<div class="form-group col-sm-3">
                            {!! Form::label('Revise Quotation Time') !!}
                            {!! Form::text('revise_time',date('h:i a'), array('class' => 'form-control ' ,'id'=>"revise_time",'required' => 'required','readonly'=>'readonly')) !!}
                            </div>

							<div class="form-group col-sm-3">
                            {!! Form::label('Generated Person') !!}
                            {!! Form::text('quatation_person',Session::get('raj_user'), array('class' => 'form-control ' ,'id'=>"quatation_person",'required' => 'required','readonly'=>'readonly')) !!}
                            </div>
                    </div>
					...
					<div class="form-group col-sm-12">
                            ...
                            {!! Form::label('Quotation No') !!}
                                 ...
                                 {!! Form::text('quatation_no',$quatation_result[0]->quatation_no, array('class' => 'form-control ' ,'id'=>"quatation_no",'required' => 'required','readonly'=>'readonly')) !!}
                            </div>
                       		<div class="form-group col-sm-3">
                            {!! Form::label('Quotation Date') !!}
                            {!! 	Form::text('quatation_date',date('d-m-Y',strtotime($quatation_result[0]->quatation_date)), array('class' => 'form-control ' ,'id'=>"quatation_date",'required' => 'required','readonly'=>'readonly')) !!}
                            </div>

							<div class="form-group col-sm-3">
                            {!! Form::label('Quotation Time') !!}
                            {!! Form::text('quatation_time',date('h:i a',strtotime($quatation_result[0]->quatation_time)), array('class' => 'form-control ' ,'id'=>"quatation_time",'required' => 'required','readonly'=>'readonly')) !!}
                            </div>

							<div class="form-group col-sm-3">
                            {!! Form::label('Quatation Generated Person') !!}
                            {!! Form::text('quatation_person',$quatation_result[0]->username, array('class' => 'form-control ' ,'id'=>"quatation_person",'required' => 'required','readonly'=>'readonly')) !!}
                            </div>
                    </div>
					...
					<!-- Revise Quatation in Change or Not Change For this Field Use -->
					<!--If More Then One Revise Quatation Then Last Revise Quatation Data Use . Else Quatatation Data Use.-->
					@if(isset($revice_quatation_result))
						<input type="hidden" name="old_quatation_product_id" id="old_quatation_product_id" value="{{$revice_quatation_result[0]->quatation_product_id}}" />
						<input type="hidden" name="old_rate" id="old_rate" value="{{$revice_quatation_result[0]->rate}}" />
						<input type="hidden" name="old_qty" id="old_qty" value="{{$revice_quatation_result[0]->qty}}" />
						<input type="hidden" name="old_specification_id" id="old_specification_id" value=	"{{$revice_quatation_result[0]->specification_id}}" />
					@else
						<input type="hidden" name="old_quatation_product_id" id="old_quatation_product_id" value="{{$quatation_result[0]->quatation_product_id}}" />
						<input type="hidden" name="old_rate" id="old_rate" value="{{$quatation_result[0]->rate}}" />
						<input type="hidden" name="old_qty" id="old_qty" value="{{$quatation_result[0]->qty}}" />
						<input type="hidden" name="old_specification_id" id="old_specification_id" value=	"{{$quatation_result[0]->specification_id}}" />
					@endif
					...
					<div class="form-group col-sm-12">
                     	<div class="form-group col-sm-12">
                            ...
                            <table width="100%"  class="table table-bordered" >
									...
									if(isset($revice_quatation_result))
									{
										$p_id_arr= explode(",",$revice_quatation_result[0]->quatation_product_id);
										$rate_arr = explode(",",$revice_quatation_result[0]->rate);
										$qty_arr = explode(",",$revice_quatation_result[0]->qty);
										$amount_arr = explode(",",$revice_quatation_result[0]->amount);
									}
									else
									{
										$p_id_arr= explode(",",$quatation_result[0]->quatation_product_id);
										$rate_arr = explode(",",$quatation_result[0]->rate);
										$qty_arr = explode(",",$quatation_result[0]->qty);
										$amount_arr = explode(",",$quatation_result[0]->amount);
									}
									...
									for($i=0; $i<count($p_id_arr);$i++)
									{
										?>
											<tr class="pending-user">
											  <td>{{$i+1}}</td>
											  <td>
												<select style="width:100%" id="quatation_product_id" name="quatation_product_id[]"  class="select2 col-md-2 quatation_product_id select2" tabindex="-1" aria-hidden="true" >
													<option value="">Select</option>
										...
										<tr>
											...
											<td>
												<div class="input-group">
													<input readonly value="<?php echo number_format((float)$quatation_result[0]->total,2,'.',''); ?>" type="text" id="total" name="total" class="form-control amountonly">
												</div>
											</td>
											<td></td>
										</tr>
										<tr>
											...
											<td>
												<div class="input-group">
													<input placeholder="Enter Discount" type="text" value="<?php echo number_format((float)$quatation_result[0]->discount,2,'.',''); ?>" id="discount" name="discount" class="form-control amountonly">
												</div>
											</td>
											<td></td>
										</tr>
										...
										<tr>
											...
											<div class="input-group">
												...
												<input type="text" id="total_amount" name="total_amount" value="<?php echo number_format((float)$quatation_result[0]->total_amount,2,'.',''); ?>" readonly class="form-control">
											</div>
											</td>
											<td></td>
										</tr>
					...
					<div class="form-group col-sm-12">
						...
						<div class="form-group col-sm-12">
						 <?php
							 //If More Then One Revise Quatation Then Last Revise Quatation Data Use in Specification. Else Quatatation Data Use.
							 if(isset($revice_quatation_result))
							 {
								$new_specification_id_arr = explode('*****',$revice_quatation_result[0]->specification_id);
								$new_spe_name_arr =explode('*****',$revice_quatation_result[0]->spe_name);
								$new_spe_value_arr = explode('*****',$revice_quatation_result[0]->spe_value);
							 }
							 else
							 {
								$new_specification_id_arr = explode('*****',$quatation_result[0]->specification_id);
								$new_spe_name_arr = explode('*****',$quatation_result[0]->spe_name);
								$new_spe_value_arr = explode('*****',$quatation_result[0]->spe_value);
							 }
...
jQuery(document).ready(function($){
	...
	$(".direct_visit").change(function() {
	if (this.value == 'yes')
	{
		$("#insentive_div").show();
		if( $("input[name='insentive']:checked").val() == 'yes')
		{
			$("#attended_div").show();
		}
		else
	...
	$('#frm').validate({
		...
		$('#frm').on('submit', function(e){
			...
			if($("#frm").valid() )
			{
				...
				//Old Quatation & Revise Quatation Any Change Then Submit Quatation Else Not Submit.
				var new_quatation_product_id =$("select.quatation_product_id").map(function(){return $(this).val();}).get().join(",");
				var new_qty =$("input[id=qty]").map(function(){return $(this).val();}).get().join(",");
				var new_rate =$("input[id=rate]").map(function(){return $(this).val();}).get().join(",");
				var new_specification_id =$("input:checkbox[id=specification_id]:checked").map(function(){return $(this).val();}).get().join(",");

				var old_quatation_product_id = $("#old_quatation_product_id").val();
				var old_qty = $("#old_qty").val();
				var old_rate = $("#old_rate").val();
				var old_specification_id = $("#old_specification_id").val();


				if(new_quatation_product_id==old_quatation_product_id && new_qty==old_qty && new_rate==old_rate && new_specification_id==old_specification_id)
				{
					alert('Old Quotation & Revise Quotation Are Same.');
					return false;
				}
