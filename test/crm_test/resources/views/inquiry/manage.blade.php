...
$(document).ready(function (){
	...
	*/
	...
	$("#inquiry_all").DataTable({
		...
		$("#inquiry_all").DataTable().column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
			cell.innerHTML = i+1;
		} );
	}).draw();
	...
	$('body').on('click','#active_inq_click',function(e){
		if (! $.fn.dataTable.isDataTable( '#inquiry_active' ) )
		{
			$("#inquiry_active").DataTable({
				...
				autoWidth:false,
				ajax:"{{ $get_active_inq_url }}"
				}).on( 'order.dt search.dt', function () {
				$("#inquiry_active").DataTable().column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
					cell.innerHTML = i+1;
				} );
			}).draw();
		}
	...
	$('body').on('click','#cancel_inq_click',function(e){
		if (! $.fn.dataTable.isDataTable( '#inquiry_cancel' ) )
		{
			$("#inquiry_cancel").DataTable({
				...
				autoWidth:false,
				ajax:"{{ $get_cancel_inq_url }}"
				}).on( 'order.dt search.dt', function () {
				$("#inquiry_cancel").DataTable().column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
					cell.innerHTML = i+1;
				} );
			}).draw();
		}
