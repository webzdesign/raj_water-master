...
class Work_report extends Controller
{
	...
	public function get_search_data(Request $request)
	{
		...
		if($role == '2' || $role == '3' || $role == '5')
		{
			...
			$data['quotation_detail'] = Data_model::db_query("select `quatation`.quatation_no,`quatation`.quatation_time,`customer_master`.prefix,`customer_master`.name,`customer_master`.mobile,`customer_master`.mobile_2,`customer_master`.mobile_3,`state_master`.`state_name`,`product_master`.product_name,`source_master`.source_name,`inquiry`.inquiry_id from `quatation` INNER JOIN `inquiry` ON `inquiry`.inquiry_id = `quatation`.inquiry_id INNER JOIN `customer_master` ON `customer_master`.customer_id = `inquiry`.customer_id INNER JOIN `state_master` ON `state_master`.state_id = `customer_master`.state_id INNER JOIN `product_master` ON `product_master`.product_id = `inquiry`.product_id INNER JOIN `source_master` ON `source_master`.source_id = `inquiry`.source_id  where `quatation`.quatation_date = '".$date."' AND `quatation`.added_by = '".$user_id."' ");
