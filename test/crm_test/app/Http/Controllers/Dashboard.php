...
class Dashboard extends Controller
{
	...
	public function index()
	{
		if($this->role_id == 1)
		{
			$data['emp_work_detail'] = Data_model::db_query("select `users`.id,`employee`.role_id,`employee`.zone_id,`employee`.name from `users` INNER JOIN `employee` ON `employee`.emp_id = `users`.emp_id where `employee`.delete_status = 0 AND `zone_id` != '' AND `users`.role_id IN (2,3,5)  ");

			$data['sales_emp_detail'] = Data_model::db_query("select `users`.id,`employee`.role_id,`employee`.zone_id,`employee`.name from `users` INNER JOIN `employee` ON `employee`.emp_id = `users`.emp_id where `employee`.delete_status = 0 AND `zone_id` != '' AND `users`.role_id IN (4,6,11)  ");
