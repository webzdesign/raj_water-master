...
class User_rights extends Controller
{
	...
	public function update_data(Request $request)
	{
		...
		for($i =0 ;$i<count($module_id); $i++)
		{
			...
			if(count($check) == 0)
			{
				$data = array('role_id'=>$role_id,'user_id'=>$user_id,'module_id'=>$module_id[$i],'add'=>$add_flg,'edit'=>$edit_flg,'delete'=>$delete_flg,'search'=>'1','print'=>'1','view'=>$view_flg,'added_by'=>$this->user_id,'added_time'=>date("Y-m-d H:i:s"));
				
				Data_model::store($this->table,$data);
			}
			else
			{
				$data = array('role_id'=>$role_id,'user_id'=>$user_id,'module_id'=>$module_id[$i],'add'=>$add_flg,'edit'=>$edit_flg,'delete'=>$delete_flg,'search'=>'1','print'=>'1','view'=>$view_flg,'updated_by'=>$this->user_id);
