...
class Follow_up extends Controller

{
	...
	public $msgName = "Follow Up";

	public $view = "follow_up";

	public $controller = "Follow_up";
...

		{
...

			{
				...
				$data['inquiry'] =  DB::table($this->foreign_table)
					...
					->select($this->foreign_table.'.*','customer_master.*','quatation.quatation_id','quatation.quatation_no','quatation.follow_up_status','quatation.q_no')
	...
	public function get_allotment(Request $request)

	{
		...
		$qdate = date('Y-m-d',strtotime($request->search_date));



		if(date('l',strtotime($qdate))==date('l',strtotime('next day',strtotime("$day_array[$h_day] 01:15"))))

		{

			$quatation_date = date('Y-m-d',strtotime($qdate." - 2 day"));

		}

		else if(date('l',strtotime($qdate)) == $day_array[$h_day])

		{

			$quatation_date = $qdate;
...

		{
			...
			$quatation_date = date('Y-m-d',strtotime($qdate." - 1 day"));
...

		{
			...
			$data['allotement_data'] =  DB::table($this->foreign_table)
...

		{
			...
			if ($extraWork) {
				...
				$transferZone = $this->zone_id.','.$transferZone;

				$where1 ="find_in_set(`quatation`.zone_id,'$transferZone') AND `quatation`.quatation_date = '".$quatation_date."' ";
			} else {
				$where1 ="find_in_set(`quatation`.zone_id,'$this->zone_id') AND `quatation`.quatation_date = '".$quatation_date."' ";
			}
		...
		return view($this->view.'/allotment_data',$data);
...

	{
		...
		$quatation_date = date('Y-m-d',strtotime($qdate));

		if($this->role_id==1)

		{

			$data['follow_up_data'] =  DB::table($this->foreign_table)

				->select($this->foreign_table.'.*','customer_master.*','quatation.quatation_id','quatation.quatation_no','follow_up.follow_up_status')

				->join('customer_master', 'customer_master.customer_id', '=', $this->foreign_table.'.customer_id')

				->join($this->table, $this->table.'.'.$this->foreign_id, '=', $this->foreign_table.'.'.$this->foreign_id)

				->join('quatation', 'quatation.'.$this->foreign_id, '=', $this->foreign_table.'.'.$this->foreign_id)

				->where($this->table.'.next_followup_date','=',$quatation_date)
...

		{
			...
			$empId = DB::table('users')->where('id', $this->user_id)->first();
			$extraWork = DB::table('emp_transfer')->where('transfer_emp_id', $empId->emp_id)->where('transfer_from_date', '<=', $quatation_date)->where('transfer_to_date', '>=', $quatation_date)->first();
			...
			if ($extraWork) {
				...
				$where1 ="find_in_set(`quatation`.zone_id,'$transferZone') AND `follow_up`.next_followup_date = '".$quatation_date."' ";
			} else {
				$where1 ="find_in_set(`quatation`.zone_id,'$this->zone_id') AND `follow_up`.next_followup_date = '".$quatation_date."' ";
			}


			$data['follow_up_data'] =  DB::table($this->foreign_table)

				->select($this->foreign_table.'.'.$this->foreign_id,'customer_master.name','quatation.quatation_id','quatation.quatation_no',$this->table.'.follow_up_status')
				...
				->orderBy('follow_up.follow_up_id','DESC')
...

	{
...

		{
			...
			$data['inq_follow_up'] =  DB::table($this->foreign_table)
				...
				->where($this->table.'.next_followup_date','=',$qdate)
...

		{
			...
			$empId = DB::table('users')->where('id', $this->user_id)->first();
			$extraWork = DB::table('emp_transfer')->where('transfer_emp_id', $empId->emp_id)->where('transfer_from_date', '<=', $qdate)->where('transfer_to_date', '>=', $qdate)->first();
			...
			if ($extraWork) {
				...
				$where1 ="find_in_set(`quatation`.zone_id,'$transferZone') AND `follow_up`.next_followup_date = '".$qdate."' ";
			} else {
				$where1 ="find_in_set(`quatation`.zone_id,'$this->zone_id') AND `follow_up`.next_followup_date = '".$qdate."' ";
			}




			$where ="find_in_set(`quatation`.zone_id,'$this->zone_id') AND `quatation`.quatation_date = '".$quatation_date."' ";



			$data['inquiry'] =  DB::table($this->foreign_table)

				->select($this->foreign_table.'.*','customer_master.*','quatation.quatation_id','quatation.quatation_no','quatation.q_no','quatation.follow_up_status')
			...
			$data['inq_follow_up'] =  DB::table($this->foreign_table)
				...
				->select($this->foreign_table.'.'.$this->foreign_id,'customer_master.name','quatation.quatation_id','quatation.quatation_no','quatation.q_no',$this->table.'.follow_up_status')
...

	{
...

		{
			...
			$pdf = PDF::loadView('quatation/print',$data1);
...

	{
		...
		ini_set('sendmail_path', 'E:\wamp64\sendmail\sendmail.exe');
		ini_set('sendmail_from', 'pratikdonga.ap@gmail.com');

		$inquiry_id = $request->input('inquiry_id');
		$minimum_days = Data_model::retrive('minimum_days','days',array('type'=>'mail'),'id','');
		...
		if( !empty($send_reminder_mail))
		{
			...
			if($sent == 1)
			{
				$mailDay = Data_model::retrive('minimum_days','*',array('type'=>'mail'),'type');
				$m_day=$mailDay[0]->days;

				$response[0]="success";
				$date = date('Y-m-d');
				$next_follow_date = date('Y-m-d',strtotime($date." + ".$m_day." day"));
				$follow_up_data = array(
					...
					'followup_way_id' => 2,
					'day_followup_date' => $m_day,
					'next_followup_date' => $next_f_date,
					'call_receive_remark' => 'Reminder Mail',
					'added_by'=>$this->user_id);

				$follow_up_id = Data_model::store($this->table,$follow_up_data);

				Data_model::restore($this->table,array('follow_up_status'=>1),array($this->primary_id=>$last_followup_id));
...

	{
...

		{
			...
			Data_model::restore('inquiry',array('hot_list'=>1),array('inquiry_id'=>$inquiry_id));
...

	{
		...
		//Data_model::restore($this->table,array('follow_up_status' => '1'),array('inquiry_id' => $inquiry_id));

		DB::table($this->table)
			->where('inquiry_id', $inquiry_id)
			->where('next_followup_date','<',$next_followup_date)
            ->update(['follow_up_status' => '1']);
...

	{
		...
		$customer_quot_no = $request->input('customer_quot_no');
		...
		$massage = $customer_quot_no." ".$getmsg[0]->format;
	...
	public function get_project_city(Request $request)
	{
		$project_state_id = $request->input('project_state_id');
		$get_city = Data_model::retrive('city_master','*',array('state_id'=>$project_state_id,'delete_status'=>0),'city_name','ASC');
